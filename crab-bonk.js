 ...
 * - Supports bottomOffset so fixed bottom bars do not cover crabs
 */
(function () {
  "use strict";
  ...
  function tick(t) {
    ...
    spawnTimer += dt;
    if (spawnTimer >= CFG.spawnEveryMs) {
      spawnTimer = 0;
      spawnCrab();
    }
  ...
  function init() {
    ...
    document.addEventListener("pointerdown", (e) => {
      if (clubVisible) bonkAt(e.clientX, e.clientY);
    }, { passive: true });

    window.addEventListener("resize", () => {
      for (const c of crabs) {
        c.x = clamp(c.x, 0, Math.max(0, window.innerWidth - c.size));
      }
    });
